...
const PlantShop = () => {
  ...
  // Fetch plants and categories from backend
  useEffect(() => {
    fetchPlants();
    fetchCategories();
    ...
    try {
      const orderData = {
        ...
        items: cart.map(item => ({
          ...
          price: item.price - calculateDiscountAmount(item)
        })),
      ...
      if (response.ok) {
        ...
        setOrderStatus({ success: true, orderId: order._id });
        setCart([]);
        setShowCheckout(false);
        // Refresh plants to update stock
        fetchPlants();
      } else {
        const error = await response.json();
        setOrderStatus({ success: false, message: error.error });
      }
    } catch (error) {
    ...
    return (
      ...
      onMouseLeave={(e) => {
        ...
        <div style={{ position: 'relative', marginBottom: '15px' }}>
          <img 
            src={plant.image || '/api/placeholder/250/200'} 
            alt={plant.name}
            style={{ 
              width: '100%', 
              height: '200px', 
          ...
          {orderStatus.success ? (
            <>
              ...
              <p style={{ margin: '0 0 20px 0', color: '#666' }}>
                Order ID: #{orderStatus.orderId.slice(-6)}
              </p>
              <p style={{ margin: '0 0 20px 0', color: '#666' }}>
